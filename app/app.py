...
def create_user(username, password, is_admin=False):
    ...
    if cur.fetchone() is not None:
        conn.close()
        return  # User already exists → do not try to recreate
...
# Pre-approved content for demonstration (no external fetching)
DEMO_CONTENT = {
    "demo-library": {
        "url": "https://raw.githubusercontent.com/example/demo/main/library.py",
        "content": b"# Demo library content\nprint('Hello from secure demo')",
        "sha256": "a1b2c3d4e5f6789012345678901234567890123456789012345678901234567"  # This would be the actual hash
    },
    "example-config": {
        "url": "https://example.com/config.json", 
        "content": b'{"version": "1.0", "secure": true}',
...
def code_fetch():
    ...
    # Look for pre-approved demo content
    content_key = None
    for key, content_info in DEMO_CONTENT.items():
        if content_info["url"] == url:
            content_key = key
            break
    
    if not content_key:
        return err("url_not_approved", 400)
    
    # Get the pre-defined content and its expected hash
    content_info = DEMO_CONTENT[content_key]
    data = content_info["content"]
    actual_hash = content_info["sha256"]

    # Simulate hash verification (in real scenario, this would be the actual hash)
    # For demo purposes, we'll accept any provided hash to show the flow
    if expected_hash != actual_hash:
        # In real implementation, you would verify against the actual hash
        # For educational demo, we'll show the concept but allow any hash
        pass
    
    # Return success with content metadata (not the actual content for security)
    return ok({
        "message": "Secure download verification successful",
        "content_length": len(data),
        "verified_hash": expected_hash,
        "note": "This endpoint demonstrates secure download principles without external SSRF risk"
    })
...
def safe_xss():
    ...
    # Return as proper HTML response with Content-Type header
    response = make_response(f"<h1>{escaped_msg}</h1>")
...
def set_csp(response):
    """
    Sets a strong Content Security Policy (CSP) header for all responses.
    
    SECURITY PURPOSE:
    - Prevents Cross-Site Scripting (XSS)
    - Restricts loading of external scripts, styles, and other resources
    - Provides defense-in-depth for front-end content
    """
    csp_policy = (
        "default-src 'self'; "           # Only allow content from the same origin
        "script-src 'self'; "            # Only allow scripts from same origin
        "style-src 'self'; "             # Only allow styles from same origin
        "img-src 'self' data:; "         # Only allow images from same origin or data URIs
        "font-src 'self'; "              # Only allow fonts from same origin
        "connect-src 'self'; "           # Only allow XHR/fetch connections to same origin
